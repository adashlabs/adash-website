...
<template>
  <div class="page">
    ...
    <header class="header">
      <router-link class="logo" to="/" :aria-label="`${site.name} ana sayfa`">
    ...
    <footer class="footer">
      <div>
        <router-link class="footer-logo" to="/">{{ site.name }}</router-link>
        <p>{{ site.footerText }}</p>
      </div>
...
.header { position: fixed; inset: 0 0 auto; z-index: 10; height: 118px; padding: 0 clamp(24px, 5vw, 80px); display: flex; align-items: center; justify-content: space-between; background: linear-gradient(180deg, rgba(9,10,10,.95) 0%, rgba(9,10,10,.7) 60%, transparent 100%); }
