...
export const TableHeader = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(
  ({ className = '', ...props }, ref) => (
    <thead ref={ref} className={`bg-muted/40 border-b border-border [&_tr]:border-b-0 ${className}`} {...props} />
  )
);
...
export const TableCell = React.forwardRef<HTMLTableCellElement, React.TdHTMLAttributes<HTMLTableCellElement>>(
  ({ className = '', ...props }, ref) => (
    <td
      ref={ref}
      className={`p-4 align-middle text-sm text-foreground [&:has([role=checkbox])]:pr-0 ${className}`}
      {...props}
    />
  )
);
